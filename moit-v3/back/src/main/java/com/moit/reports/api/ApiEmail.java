...
//	public String password;
	...
	public void sendMail(String subject, String content, String to) {
		/*
		 * Properties props = new Properties(); props.put("mail.smtp.host", host); // 어떤
		 * 메일 서버 쓸지. props.put("mail.smtp.auth", "true"); // 아이디/비밀번호 인증 사용 여부
		 * props.put("mail.smtp.port", "587"); // 포트 // props.put("mail.debug", "전송완료");
		 * // 전송 로그 출력
		 * 
		 ...
		 * 
		 * // 3. Session session = Session.getInstance(props, new Authenticator() {
		 * 
		 ...
		 * InternetAddress(user)); // 보내는 사람
		 * message.addRecipient(Message.RecipientType.TO, new InternetAddress(to)); //
		 * 받는 사람 message.setSubject( subject ); // 제목 message.setText( content +
