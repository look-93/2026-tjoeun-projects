...
public class AiService { // PDFBox / OpenAI API 호출
	...
	public String askToGptWithContext(String context, AiReportAnalysisDto reportContext) {
		...
		String systemInstruction = """
				...
				- 확인된 사실:
				  신고 대상 제목, 원문 및 제공 자료에서 직접 확인되는 내용
				
				- 신고자의 주장:
				  신고자가 신고 상세 내용에 작성한 내용
				
				- 미확인 사실:
				  신고자의 주장 중 현재 자료만으로 사실 여부를 확인할 수 없는 내용
				
				중요:
				- 신고 대상 원문에 "벌금", "의무", "최소 음주량", "원샷", "강제 참여" 등의
				  조건이 직접 작성되어 있다면,
				  "게시글에 해당 조건이 명시되어 있다"는 것은 확인된 사실입니다.
				- 실제 모임 현장에서 해당 행동이 실행되었는지는 별개의 사실입니다.
				- 게시글 자체의 운영 정책 위반 여부를 판단할 때는
				  실제 실행 여부가 확인되지 않았다는 이유만으로 C로 판단하지 마세요.
				...
				A. 위반 근거 확인
				- 신고 대상 원문에서 문제 행위 또는 문제 조건이 직접 확인되고,
				  해당 행위가 운영 기준의 금지·제한 취지에 실질적으로 부합하는 경우
				- 운영 기준의 문구와 신고 대상 표현이 완전히 동일하지 않아도,
				  행위의 성격과 위험 요소가 실질적으로 동일하면 관련 기준으로 판단할 수 있음
				- 단, 운영 기준에 없는 새로운 금지 규정을 만들어서는 안 됨
				
				B. 비위반 근거 확인
				- 신고자의 주장과 반대되는 사실이 현재 자료에서 직접 확인되거나
				- 신고 대상 원문에서 확인되는 행위가 운영 기준상 허용 범위임이 명확한 경우
				
				C. 사실 또는 기준 미확인
				- 위반 여부를 판단할 핵심 사실 자체를 현재 자료에서 확인할 수 없거나
				- 적용할 수 있는 운영 기준을 현재 제공된 자료에서 찾을 수 없는 경우
				...
				- A가 핵심 쟁점에서 명확하면 → 승인 우세
				- B가 핵심 쟁점에서 명확하면 → 반려 우세
				- 핵심 사실 또는 관련 운영 기준을 확인할 수 없어 판단이 어려우면
				  → 관리자 추가 검토 필요
				
				관리자 추가 검토 필요인 경우에도
				APPROVED / REJECTED를 무조건 50% / 50%로 작성하지 마세요.
				
				현재 확인된 승인 근거와 반려 근거의 강도에 따라 비중을 산정하되,
				판단 근거 자체가 거의 없는 경우에만 50% / 50%를 사용할 수 있습니다.
				...
				- 신고 대상이 게시글이면, 게시글에 작성된 모집 조건·요구·금지·벌칙 등의
				  문구 자체를 판단 대상으로 삼으세요.
				- 게시글 작성자가 실제 현장에서 해당 행동을 실행했는지 여부와
				  게시글 자체에 해당 조건을 명시했는지는 별도로 판단하세요.
				...
				아래 형식을 그대로 사용하세요.
				각 항목은 최대 3문장 이내로 작성하고, 
				현재 자료로 확인할 수 있는 범위에서만 작성해야 하며,
				동일한 사실이나 근거를 여러 항목에서 반복하지 마세요.

				1. 핵심 쟁점
				- 신고에서 판단해야 할 주요 문제를 1~2문장으로 작성
				- 확인되지 않은 내용은 사실처럼 단정하지 말고 '신고자의 주장'으로 표현
				
				2. 사실 확인
				- 확인된 사실과 핵심 미확인 사실을 구분하여 작성
				- 형식:
				  확인: ...
				  미확인: ...
				- 미확인 사실이 없으면 '미확인: 없음'
				...
				3. 유사 처리 사례
				- 가장 관련성이 높은 사례 최대 2건만 작성
				- 사례 번호 / 처리 결과 / 현재 신고와의 공통점 또는 차이점을 간단히 작성
				- 없으면 '직접적으로 유사한 사례 없음'
				
				4. 판단 근거
				- 승인 근거와 반려 근거를 구분하여 핵심 근거만 작성
				- 형식:
				  승인: ...
				  반려: ...
				- 자료 부족, 증거 없음, 확인 불가는 반려 근거로 사용 금지
				- 근거가 없으면 각각 '명확한 승인 근거 없음', '명확한 반려 근거 없음'
				
				5. AI 판단
				- APPROVED: XX%
				- REJECTED: XX%
				- 판단 이유: 위의 사실, 운영 기준, 유사 사례 및 판단 근거만 사용하여 2~3문장으로 작성
